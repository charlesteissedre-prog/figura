"""Pitch + LTAS sweep on the male Vevo 2 variants."""
...
VARIANTS = {
    "raw vevo2-out":   REPO / "outputs" / "_male_vevo2_nopitch.wav",
    "PSOLA 1-pass":    REPO / "outputs" / "_male_vevo2_nopitch_psola_a100.wav",
    "PSOLA 4-iter":    REPO / "outputs" / "_male_vevo2_nopitch_psola_iter.wav",
}
SRC = REPO / "examples" / "male" / "source.wav"
TGT = REPO / "examples" / "male" / "target.mp3"
...
def main():
    ...
    src_l = src_ltas[mask]
    tgt_l = tgt_ltas[mask]
    print(f"source mean F0: {src_mean:.1f} Hz   target mean F0: {mean_f0(tgt):.1f} Hz")
    print(f"src<->tgt LTAS sim (reference): {cos_sim(src_l, tgt_l):.4f}\n")

    print(f"{'variant':18s}  {'mean F0':>9s}  {'st vs src':>10s}  "
    ...
    for name, path in VARIANTS.items():
        ...
        print(f"{name:18s}  {mf0:7.1f} Hz  {st_vs_src:+8.2f} st  "
